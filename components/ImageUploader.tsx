...
import { isValidImageType, isValidFileSize, formatBytes } from '@/lib/image-utils';
...
export function ImageUploader({ onImageSelect, currentImage, disabled = false, onQuickCompress }: ImageUploaderProps) {
    ...
    (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
      ...
      if (rejectedFiles.length > 0) {
        ...
        if (rejection.errors?.[0]?.code === 'file-too-large') {
          ...
          errorMessage = 'Invalid file type. Only JPG, PNG, WEBP, and AVIF are supported.';
        }
        ...
        <div className="rounded-lg bg-muted p-3 text-xs">
          <div className="flex items-start gap-2">
            ...
            <p className="text-muted-foreground">
              Your image is ready to compress. Adjust the settings below and click
              &quot;Compress Image&quot; to start.
            </p>
          </div>
        </div>
      )}
